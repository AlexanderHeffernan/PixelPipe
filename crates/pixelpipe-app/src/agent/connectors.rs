...
const AGENT_PROMPT: &str = concat!(
    ...
    "If an Amp image tool returns a private https://ampcode.com/user-content/attachments/... URL, ",
    "return that URL as attachment_url and let PixelPipe import it after your response. Do not try to ",
    "download Amp attachments yourself. For any other remote result, immediately use the local ",
    "rendering fallback. Local candidates use path and sha256; Amp attachment candidates use only ",
    "attachment_url. Never use arbitrary remote URLs. ",
    ...
    "one focused valid change. Your final response must be only strict pixelpipe.agent-response/v1 JSON. ",
    "For generation use exactly this shape, with no other fields: ",
    r#"{"schema":"pixelpipe.agent-response/v1","adapter":{"adapter":"amp","provider":"reported provider","model":"reported model","capabilities":["generate_references"]},"result":{"type":"generated_references","candidates":[{"id":"lowercase-hyphen-id","attachment_url":"https://ampcode.com/user-content/attachments/file.png"}]}}. "#,
    "Replace values and include all three candidates. Do not add request id, operation, asset, identity, ",
    "image_generation, or commentary fields. Candidate IDs must use lowercase hyphens; local paths must ",
    "be relative to output_directory with exact SHA-256 hashes. Never ",
