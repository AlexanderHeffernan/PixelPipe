...
fn cli_init_create_and_inspect_share_json_contract() {
    ...
    assert_eq!(init["schema"], "pixelate.project/v1");

    let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../pixelate-core/tests/fixtures/m1/tiny-raster.json");
    let revision = run(&[
        "revision",
        "create",
        "--root",
...
fn cli_updates_and_exports_an_existing_sprite_with_its_current_style() {
    let project = tempdir().expect("project tempdir");
    let root = project.path().to_str().expect("UTF-8 project path");
    pixelate_app::open_project(pixelate_app::OpenProject {
    ...
    let fixtures = Path::new(env!("CARGO_MANIFEST_DIR")).join("../pixelate-core/tests/fixtures/m2");
...
fn cli_initializes_pre_revision_asset_and_project_resources() {
    ...
    let fixtures = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/m6");
    ...
    assert!(
        show["recipes"]
            ...
            .expect("recipe list")
            .iter()
            .any(|recipe| recipe["id"] == "synthetic-flare")
    );
...
fn cli_converts_synthetic_png_through_the_application_use_case() {
    ...
    let stored_reference = project
        .path()
        .join(".pixelate/assets/synthetic-pickup/references/selected")
        .join(format!(
            "{}.png",
...
fn cli_refines_branches_compares_and_records_review_without_mutating_parents() {
    ...
    let raster = crate_root.join("../pixelate-core/tests/fixtures/m1/tiny-raster.json");
    let fixtures = crate_root.join("tests/fixtures/m3");
    let created = run(&[
        "revision",
        "create",
        "--root",
...
fn assert_invalid_patch_is_atomic(project: &tempfile::TempDir, root: &str) {
    ...
    fs::write(
        &invalid_patch,
        r#"{"schema":"pixelate.patch/v1","edits":[{"x":99,"y":0,"index":1}]}"#,
    )
    ...
    assert!(
        !project
            .path()
            .join(".pixelate/assets/signal-flare/revisions/r000004")
            .exists()
    );
}
...
fn run_failure(arguments: &[&str]) -> Value {
    let output = Command::new(env!("CARGO_BIN_EXE_pixelate"))
        ...
        .output()
        .expect("run pixelate");
    assert!(!output.status.success(), "pixelate unexpectedly succeeded");
