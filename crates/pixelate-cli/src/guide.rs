...
pub(crate) fn agent_guide(root: &Path) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    ...
    Ok(json!({
        ...
        "rules": [
            ...
            "Adoption imports a verified internal source copy and links the existing project image without rewriting it. Moving assets or folders refuses collisions and never changes stable asset IDs or revision history.",
        ...
        "capabilities": {
            ...
            "adopt_project_image": "pixelate asset adopt --root . --path <project-relative-image> --asset <asset-id> --brief '<brief>'",
