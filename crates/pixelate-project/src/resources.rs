...
use pixelate_core::{Palette, stable_json};
...
impl ProjectStore {
    ...
    pub fn palettes(&self) -> Result<Vec<(String, Palette)>, ProjectError> {
        let path = self.root.join(".pixelate/palettes");
        if !path.is_dir() {
            return Ok(Vec::new());
        }
    ...
    pub fn store_conversion_recipe(
        ...
        let _lock = self.lock()?;
        let recipes = self.root.join(".pixelate/recipes");
    ...
    pub fn conversion_recipes(&self) -> Result<Vec<ConversionRecipeDocument>, ProjectError> {
        let path = self.root.join(".pixelate/recipes");
        if !path.is_dir() {
            return Ok(Vec::new());
        }
...
fn validate_conversion_recipe(recipe: &ConversionRecipeDocument) -> Result<(), ProjectError> {
    ...
    if recipe.preview_scale == 0 {
        return Err(ProjectError::Core(
            pixelate_core::CoreError::InvalidPreviewScale,
        ));
    }
    Ok(())
}
